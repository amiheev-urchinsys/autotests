...
from data.constants import DOMAIN_STAGE_URL, SUCCESS_POPUP_TITLE
...
def test_invite_and_register_new_company_user(context_and_playwright):
    ...
    on_company_page.popups.invite_company_user_email_address_input.fill("")
    on_company_page.popups.invite_company_user_role_selector.click()
    on_company_page.popups.invite_company_user_role_selector_list_administrator_point.click()
    on_company_page.popups.invite_button.click()
    # on_admin_console_page.send_invite_new_company_owner_form(temp_email_data["email"])
    # # Verification Success popup is present
    # expect(on_admin_console_page.companies_tab.success_popup.title).to_have_text(SUCCESS_POPUP_TITLE)
    # # Steps to get register link from email
